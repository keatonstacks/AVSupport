...
(() => {
    ...
    {
        ${/* The array of 256 colors for "jupiterA" is quite large. */''}
${/* (Pasted exactly as you provided) */''}
        vec2 imageSize = vec2(16, 16);
        const vec3 image[256] = vec3[](
            // -- omitted here for brevity in explanation --
            // [PASTE EXACT "jupiterA" array from your snippet]
            vec3(0.906,0.780, 0.678),vec3(0.906,0.780, 0.678),/* ... etc ... */vec3(0.816,0.682, 0.600)
        );
    ...
    {
        ${/* array of 256 colors for "jupiterB" */''}
${/* (Pasted exactly as you provided) */''}
        vec2 imageSize = vec2(16, 16);
        const vec3 image[256] = vec3[](
            // -- omitted here for brevity in explanation --
            // [PASTE EXACT "jupiterB" array from your snippet]
            vec3(0.420,0.220, 0.094)
        );
        int xIndex = int(floor(uv.x * imageSize.x - 0.5));
        int yIndex = int(floor(uv.y * imageSize.y - 0.5));
        vec3 sample00 = image[WrapIndexY(yIndex) * 16 + WrapIndexX(xIndex)];
